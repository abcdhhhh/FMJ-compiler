...
static void UD_enter(G_node n, UDinfo info) { G_enter(UDTable, n, info); }

static UDinfo UD_lookup(G_node n) { return G_look(UDTable, n); }
...
static void LT_enter(Temp_label l, G_node n) { TAB_enter(LNTable(), l, n); }

Temp_tempList FG_def(G_node n) { return UD_lookup(n)->defs; }

Temp_tempList FG_use(G_node n) { return UD_lookup(n)->uses; }

bool FG_isMove(G_node n) { return UD_lookup(n)->isMove; }
        ...
        if (instr != NULL) {
            ...
            switch (instr->kind) {
                case I_OPER:
                    ...
                    break;
                case I_LABEL:
                    // 2.2) label should be also saved in the label-node list
                    // for (II)
            ...
            UD_enter(curr, UDInfo(uses, defs,
                                  type == IT_MOVE && strstr(instr->u.MOVE.assem,
                                                            "S") == NULL));

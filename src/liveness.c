...
static void INOUT_enter(G_node n, inOut info) { G_enter(InOutTable, n, info); }

// Lookup the inOut info
static inOut INOUT_lookup(G_node n) { return G_look(InOutTable, n); }
...
static Temp_tempList TempList_union(Temp_tempList tl1, Temp_tempList tl2) {
    for (; tl2 != NULL; tl2 = tl2->tail) tl1 = TempList_add(tl1, tl2->head);
    return (tl1);
}
...
static bool LivenessInteration(G_nodeList gl) {
    bool changed = FALSE;
    gi++;
    while (gl != NULL) {
        G_node n = gl->head;
        ...
        Temp_tempList in =
            TempList_union(FG_use(n), TempList_diff(FG_Out(n), FG_def(n)));

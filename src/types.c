...
Ty_ty Ty_Record(S_symbol parent, Ty_fieldList fields, Ty_methodList methods) {
    ...
    p->kind = Ty_record;
    p->u.record.parent = parent;
    p->u.record.fields = fields;
    p->u.record.methods = methods;
    return p;
}
...
Ty_methodList Ty_MethodList(Ty_method head, Ty_methodList tail) {
    Ty_methodList p = checked_malloc(sizeof(*p));
    p->head = head;
    p->tail = tail;
    return p;
}
...
static char str_ty[][12] = {"ty_record", "ty_nil",  "ty_int", "ty_bool",
